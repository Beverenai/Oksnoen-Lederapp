...
import { Home, ChevronDown, ChevronUp, CheckCircle2, AlertTriangle, Loader2 } from 'lucide-react';
...
export function CabinAssignmentStatus() {
  ...
  const [stats, setStats] = useState<CabinAssignmentStats>({ total: 0, withCabins: 0, withoutCabins: 0, percentage: 0 });
  ...
  const loadCabinAssignmentStats = async () => {
    ...
    try {
      ...
      const total = withCabins.length + withoutCabins.length;
      setStats({
        total,
        withCabins: withCabins.length,
        withoutCabins: withoutCabins.length,
        percentage: total > 0 ? Math.round((withCabins.length / total) * 100) : 0,
      });
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 rounded-lg bg-muted/50 text-center">
            ...
            <div className="text-xs text-muted-foreground">Aktive ledere</div>
          </div>
