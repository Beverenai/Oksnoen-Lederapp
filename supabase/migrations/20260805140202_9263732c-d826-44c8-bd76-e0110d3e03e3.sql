...
BEGIN
  ...
  SELECT array_agg(t.leader_id) INTO _revived
  FROM (
    SELECT mp.leader_id
  ...
  UPDATE public.murder_players
   ...
   WHERE game_id = _game_id AND leader_id = ANY(_revived);
...
REVOKE ALL ON FUNCTION public.revive_and_reshuffle_murder(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.revive_and_reshuffle_murder(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.revive_and_reshuffle_murder(integer) TO service_role;